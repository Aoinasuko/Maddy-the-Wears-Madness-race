...
{
	public class Maddy_WorldComponent : WorldComponent
	{

		public Maddy_WorldComponent(World world)
			: base(world)
		{
		}
		...
		public override void WorldComponentTick()
		{
			...
			{
				// 現在のバージョン
				if (Maddy_Config.Updatever < Maddy_Config.ver)
				{
					Maddy_Config.Updatever = Maddy_Config.ver;
					Dialog_Update dialog = new Dialog_Update();
					Find.WindowStack.Add(dialog);
					LoadedModManager.GetMod<Maddy_Settings>().WriteSettings();
				}
			}
		}
	...
	public class Dialog_Update : Window
	{
		...
		{
			Texture2D texture_update = ContentFinder<Texture2D>.Get("Maddy/UI/Update");
			Texture2D texture_nasuko = ContentFinder<Texture2D>.Get("Maddy/UI/Madolche");
			...
			listingStandard.Label("Maddy.Update.Title".Translate());
			...
			listingStandard.Label("Maddy.Update.label".Translate());
			listingStandard.End();
